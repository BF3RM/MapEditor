...
function DrawTable() {
	var table = $('#blueprint_list').DataTable({
		select: true,
		"columnDefs": [{
			"targets": [0],
			"visible": false,
	...
	var tabletwo = $('#entity_list').DataTable({
		select: true,
		"columnDefs": [{
			"targets": [0],
			"visible": true,
	...
	$('#entity_list tbody').on('click', 'tr', function() {
		...
		//this is temporary, i dunno how to do it properly
...
function drawRow(id, rowData) {
	var row = $("<tr />")
	$("#blueprint_list tbody").append(row); //this will append tr element to table... keep its reference for a while since we will add cels into it
	row.append($("<td>" + rowData.instanceGuid + "</td>"));
	row.append($("<td>" + rowData.partitionGuid + "</td>"));
	row.append($("<td>" + rowData.typeName + "</td>"));
...
function OnSpawnedEntity(p_ID, p_InstanceName) {
	var row = $("<tr />")
	$("#entity_list tbody").append(row); //this will append tr element to table... keep its reference for a while since we will add cels into it
	row.append($("<td>" + p_ID + "</td>"));
	row.append($("<td>" + p_InstanceName + "</td>"));
}
...
function AddMarker(){
	DeleteMarker();
	$('#page').append('<div id = \"marker\"></div>');
	$('#marker').css({left: -50, top: -50});
}
