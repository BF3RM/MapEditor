...
export class LinearTransform {
	...
	constructor(
		left: Vec3 = new Vec3().left,
		up: Vec3 = new Vec3().up,
		forward: Vec3 = new Vec3().forward,
		trans: Vec3 = new Vec3()
	) {
		this.left = left;
		this.up = up;
	...
	public toTable() {
		return {
			left: this.left.toTable(),
			up: this.up.toTable(),
			forward: this.forward.toTable(),
			trans: this.trans.toTable()
		};
	}
	...
	public setFromString(matrixString: string) {
		...
		this.left = new Vec3(Number(matrix[0]), Number(matrix[1]), Number(matrix[2]));

		this.up = new Vec3(Number(matrix[3]), Number(matrix[4]), Number(matrix[5]));

		this.forward = new Vec3(Number(matrix[6]), Number(matrix[7]), Number(matrix[8]));

		this.trans = new Vec3(Number(matrix[9]), Number(matrix[10]), Number(matrix[11]));
		this.UpdateMeta();
		return this;
	}
	...
	public setFromMatrix(matrix: Matrix4) {
		this.left.set(matrix.elements[0], matrix.elements[1], matrix.elements[2]);

		this.up.set(matrix.elements[4], matrix.elements[5], matrix.elements[6]);

		this.forward.set(matrix.elements[8], matrix.elements[9], matrix.elements[10]);

		this.trans.set(matrix.elements[12], matrix.elements[13], matrix.elements[14]);
		this.UpdateMeta();
		return this;
	}
