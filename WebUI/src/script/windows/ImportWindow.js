...
export class ImportWindow {
	...
	Initialize() {
		...
		Object.values(this.widgets).forEach(function (widget) {
			widget.component = function (container, state) {
				...
				this._container.getElement().html(widget.el.dom);
			};
			scope.layout.registerComponent(widget.name, widget.component);
			console.log('Registered component: ' + widget.name);
		});
	...
	OnSuperBundleSelected(superBundle) {

	}

	OnBundleSelected(bundleName, showOnly = false) {
		console.log(bundleName);
		let scope = this;
		setTimeout(() => {
			scope.GeneratePathData(bundleName);
		});
		if (showOnly) {
			return;
		}
		let bundleData = editor.fbdMan.getBundle(bundleName);
	...
	OnPathSelected(path) {
		...
		setTimeout(() => {
			scope.GenerateContentData(path);
		});
		console.log('path');
	}

	SetSuperbundle(superBundleName) {
		console.log('Changing SuperBundle to ' + superBundleName);
		this.superBundle = editor.fbdMan.getSuperBundle(superBundleName);

		let scope = this;

		setTimeout(() => {
			scope.GenerateBundleData(superBundleName);
		});
		console.log('super');
	}
