		...
		<template v-if="reference">
			<template v-if="instance">
				<div class="ReferenceBox" :class="expanded ? 'expanded' : ''" @click="expanded = !expanded">
					...
					<div class="path">
						{{ cleanPath }}<span class="guid">{{ guid }}</span>
					</div>
			...
			<template v-else>
				{{ cleanPath }} <span class="Guid">{{ guid }}</span> - {{ reference.partitionGuid }} /
			...
			if (this.$data.instance.typeName === 'ReferenceObjectData') {
				this.$data.instance.fields.blueprint.value.getPartition().data.then(() => {
					this.$data.referenceObjectBlueprint =
						this.$data.instance.fields.blueprint.value.getInstance().fields.name.value;
