...
def main():
    ...
    spawn_at(A.ADDR, A.BP, 'ED170122-7777-0000-0000-9AAA00000001', 0.0)
    time.sleep(A.SETTLE + 4)

    g_a = guid_ending('000001')
    if g_a is None:
        print('SETUP: vehicle A did not register'); return 2

    pos = A.positions()
    if '000001' not in pos:
        print('SETUP: no position for A (%s)' % json.dumps(pos)); return 2

    y_a = float(pos['000001'])
    ...
    spawn_at(A.ADDR, A.BP, 'ED170122-7777-0000-0000-9AAA00000002', 0.0)
    time.sleep(A.SETTLE + 4)

    pos = A.positions()
    if '000002' not in pos:
        print('SETUP: no position for B (%s)' % json.dumps(pos)); return 2

    y_b = float(pos['000002'])
