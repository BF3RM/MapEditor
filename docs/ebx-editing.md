...
| Remove *one* element / insert at index / reorder | ❓ **no API observed** | must rebuild via `clear()` + `add()`; see §1.2 |
...
| Point a blueprint's `objects` entry at another partition | ❌ **never** — 0 of 195 440 in vanilla | hard structural invariant; the instance must be copied *into* the partition (§2.1b) |
| Soft-remove an element via `excluded = true` | ✅ for 126 of 475 `GameObjectData` types | only `ReferenceObjectData`/`ComponentData`/`SocketData` declare it (§5.3) |

Three findings dominate the design:

1. **"Add a component" is three different operations in Frostbite, not one** (§2.1) — and 7 998
   partitions have an `ObjectBlueprint` primary instance, which has no `objects` array at all.
2. **Array position is load-bearing in at least four separate ways** (§2.1c): `indexInBlueprint` ==
   position, a `65535` sentinel, `physicsPartInfos[].partComponentIndex` pointing into
   `components[]` (in 6 577 files), and parallel `times`/`values` tracks. A naive
   append/remove/reorder produces data that loads, renders, and is silently wrong.
3. **Connection endpoints are mostly *cross*-partition** (§7.5) — `F→F` is the largest bucket for
   events and links — so a same-partition-scoped endpoint picker is not enough, and deleting an
   object can break a bus in a partition that the object has no back-reference to.

Two things this investigation found **already built and unused**: the field-id hash table shipped in
`data.zip` (§1.5) and a searchable type-filtered partition list in `ExplorerComponent.vue` that is
90 % of the reference picker (§3.3).
...
### 2.1b What the corpus says an `objects` entry actually is

Measured over the Venice-EBX dumps (73 475 partition files):

- **`objects` members are bare, same-partition instance guids. Always.** Syntactically: 0 of
  **195 440** members contain a `/` (the cross-partition `<path>/<GUID>` form). Semantically, on the
  ~20 000-file `Objects/` + `Gameplay/` + `Levels/` subset: **18 738 members resolved against an
  instance header in the same file, 0 unresolved.**

  > **This is a hard structural invariant and it constrains the whole feature: a blueprint's object
  > list never leaves its own partition.** Cross-partition composition happens exactly one level
  > down, through `ReferenceObjectData.blueprint`, which *does* use the `<path>/<GUID>` form.
  > So "add a component" cannot mean "point at an instance in another partition" — it means
  > "create/clone an instance **into this partition** and list it".

- **A raw entity goes straight into `objects`; a `ReferenceObjectData` is only needed to instantiate
  another blueprint.** The minimal light prefab is two instances —
  `Objects/CeilingLight_02/ceilinglight_02_yellow_dest_nongroupable_autogen.txt` is a
  `PointLightEntityData` plus a `PrefabBlueprint` whose `Objects::array` has that one guid.
  `Objects/ElectricWire_01/ElectricWireLamp_WithLight.txt:54-56` shows the mixed case: `member(0)` is
  a `ReferenceObjectData` pointing cross-partition at a mesh prefab, `member(1)` is a
  `SpotLightEntityData` sitting directly in the array.

  > **"Add a light to this prefab" is therefore a two-instance operation, not a wrapper dance:**
  > construct/clone a `PointLightEntityData` and `objects:add()` it. That is the simplest possible
  > shape for the motivating example.

- Concrete element types are an open set. Counting resolved members over that subset (18 738):
  `ReferenceObjectData` 6 491, `WorldPartReferenceObjectData` 3 542, `SubWorldReferenceObjectData`
  561, `CharacterSpawnReferenceObjectData` 427, `DelayEntityData` 404, `AlternateSpawnEntityData` 363,
  `StaticModelGroupEntityData` 334, **`PointLightEntityData` 332**, `EffectReferenceObjectData` 303,
  … **`SpotLightEntityData` 147**. Roughly 60 % are some `*ReferenceObjectData`; the rest are leaf
  entity data placed directly.

- **`objects` is not a unique field name.** `HealthStateData.objects` (12 260 members) and
  `BreakablePartComponentData.objects` (3 958) are unrelated fields that happen to share it.
  A design that keys behaviour off the field name alone will mis-handle those; key off the owning
  type.

- **The `InterfaceDescriptorData` is NOT listed in `objects`.** On
  `Gameplay/Level_Setups/Components/CapturePointPrefab.txt`, `Objects::array` has 7 members
  (`:373-380`) and the descriptor `B5571DDE-…` is not among them — it is reachable only via
  `EntityBusData.descriptor` (`:368`). **Any UI that enumerates "what is in this prefab" from
  `objects` will silently omit the thing that defines the prefab's interface.**
  It may also be a *subclass* (`AssemblyDescriptorData` in
  `Objects/ElectricWire_01/ElectricWireLamp_WithLight.txt:2-3`), so match on ancestry, not on the
  type name.

### 2.1c Hidden index coupling — the sharpest hazard in the whole feature

Three separate mechanisms make array **position** load-bearing. Any add/remove/reorder that ignores
them produces data that loads, renders, and is silently wrong.

**(i) `indexInBlueprint` *is* the position in `objects`.** Over a 4 000-file sample:
**2 861 of 2 861** `objects` members had `indexInBlueprint == array position`, zero mismatches, zero
unresolved. `CapturePointPrefab.txt` is exact: `member(0)`→`indexInBlueprint 0`, … `member(6)`→`6`.
(Note the file's *textual* instance order is alphabetical by type name, so index 5 and 6 appear
before 3 and 4 in the file — the array is the truth, not the layout.)

> **Insert, remove, or reorder ⇒ renumber `indexInBlueprint` on every affected instance, in the same
> command.** This is not optional and it is not currently anywhere in the codebase.

**(ii) `indexInBlueprint == 65535` is a sentinel meaning "deliberately not in `objects`."**
`0xFFFF`. Present in **9 252 files** — e.g. the `PhysicsEntityData` at
`Objects/AdFrame_01/AdFrame_04.txt:70-77`. Do not treat it as a real index and do not compact it
during renumbering.

**(iii) `PhysicsPartInfo.partComponentIndex` indexes into `components[]`,
and `.healthStateIndex` into `healthStates[]`.** `Objects/ShopShowcase_01.txt:524-549` shows 5
components and 5 `PhysicsPartInfo` entries with `PartComponentIndex 0..4`.
**`PartComponentIndex` appears in 6 577 files** — this is a mainstream pattern, not a corner case.

> **Removing or reordering an element of `components` silently invalidates every
> `partComponentIndex` past the edit.** A remove-component feature that does not fix these up will
> mis-assign physics parts, and nothing will report an error.

**(iv) `runtimeComponentCount` is NOT `#components`.** Corpus-wide: 18 163 equal, **366 unequal**.
`Objects/Silo_01/Silo_M_01.txt:332-335` has 2 members and `RuntimeComponentCount 1` (the second
member is a `SoundEntityData`, not a runtime component). Larger divergences exist —
`Vehicles/GrowlerITV/GrowlerITV.txt:7078` has `rcc=99` against 19 members.

> **Do not auto-derive `runtimeComponentCount` from array length.** On append, the safest behaviour
> is to leave it alone and surface it as an editable field with a warning, rather than to guess.
> Whether the engine tolerates a stale count is unverified (§10 Q9).

**(v) `PropertyTrackData.times` ↔ `values` are strict parallel arrays** (7↔7 in
`Gameplay/GameModes/XP4/Scavenger_AmmoCrate.txt:37-58`; no genuine length mismatch found corpus-wide).
A generic array editor **must not** let you append to one without the other. This is the clearest
argument for the editor knowing about a small table of "coupled arrays" rather than treating every
array as independent.
...
preferring the **component** form in the picker's defaults (§2.2, and §11 Phase 3 item 12): it makes the object
...
And the coupled state from §2.1c, which is the part most likely to be forgotten:

- `indexInBlueprint` — allocate it, respecting §7.4's unresolved question. Never `65535` for
  something you are actually putting in `objects`.
- `runtimeComponentCount` on the owning entity, if appending to `components` — **do not derive it
  from `#components`** (366 vanilla counterexamples).
- `physicsPartInfos[].partComponentIndex` on the owning entity, if the `components` array shifted.
...
1. **Within the currently inspected partition.** `PartitionSerializer` already serializes every
   ...
   no ext change whatsoever.**

   This is **sufficient** for `objects` / `components` appends, because those arrays are
   same-partition by invariant (§2.1b). It is **not** sufficient for connections — see §7.5, where
   the corpus shows cross-partition endpoints are the *majority* case.
...
a connection genuinely requires array mutation.

**Nulling an endpoint instead is only partly attested by vanilla data.** Attributing every
`Source *nullGuid*` / `Target *nullGuid*` in the corpus to its enclosing struct:

- **`LinkConnection.source` is nullable — 705 occurrences across 230 files**, and 705 of 705 pair
  with `SourceFieldId 0`. E.g. `Levels/COOP_002/COOP_002.txt:1889-1893`,
  `Gameplay/Prefabs/CoOp_FailSafe.txt:81-85`. These read as deliberate "unbound link slot"
  placeholders.
- **`PropertyConnection` and `EventConnection` never have a null `source` or `target`. Not once.**
- **No connection of any kind ever has a null `target`. Not once.**

> So "disconnect by nulling" is a shape the shipping data uses **only** for `LinkConnection.source`.
> A validator should permit exactly that (with `sourceFieldId = 0`) and reject every other null
> endpoint. For property and event connections, removal must be a real array removal.

*(Caution for anyone re-running this: a name-only grep for `Source *nullGuid*` returns 2 371 and
looks like a pile of broken event connections. 1 666 of those belong to `VoiceOverDialogTake`, an
unrelated type. Attribute per enclosing `member(N)::Type`. Also, the dumps are CRLF — a naive
comparison against `*nullGuid*` fails on the trailing `\r`.)*
...
### 7.4 `indexInBlueprint` — an unresolved conflict between two measurements

This document originally assumed `indexInBlueprint` was an identity to be preserved. **The corpus
says the opposite, and it also disagrees with the existing wiring doc.** Both measurements are
recorded here because the discrepancy has to be resolved before any reorder ships.

| Claim | Evidence |
|---|---|
| **`indexInBlueprint` is the position in `objects`** | 2 861 of 2 861 members matched, 0 mismatches, over a 4 000-file sample of `Objects/` + `Gameplay/` + `Levels/` (§2.1c(i)). Exact on `CapturePointPrefab.txt`. |
| **`indexInBlueprint` is level-global, disjoint across world parts** | `docs/entity-wiring-and-networking.md` §1.5: MP_001 ranges `CQ_logic` 4–46, `Default` 48–73, `Buildings` 148–469, `FX` 2336–2464, `Extra` 2706–2803. |

These cannot both hold for `Buildings.txt` — a world part whose members start at 148 cannot have
`indexInBlueprint == array position`. Most likely explanations, in order of plausibility:

1. The 4 000-file sample under-weighted level world parts (it was drawn from three trees, and
   `Objects/` alone is far larger than `Levels/`), so the invariant is real for **prefabs** and
   different for **world parts**.
2. The wiring doc's ranges were read per-world-part from a level-global numbering that the objects
   arrays nonetheless index into contiguously (i.e. position + a per-part base).

**This is directly resolvable from the dumps and nobody should reorder anything until it is** — see
§10 Q10. It is a half-hour of grepping, not a game session.

Meanwhile, `LevelInjector` allocates `#p_World.objects + self.m_IndexCount + 1`
(`LevelInjector.lua:569`) with `m_IndexCount` estimated by scanning only each world part's last
element (`:381-389`) — the under-estimate that is **G8**. Whichever way Q10 resolves, **append must
allocate an index and the design must state which numbering it is appending to.**

---

### 7.5 Connection endpoints are mostly cross-partition — this kills the cheap picker

Classifying all ~165 000 connections in the corpus by endpoint locality (**L** = bare guid, same
partition as the bus; **F** = `<path>/<GUID>`, foreign; **N** = null):

| | L→L | F→F | L→F | F→L | N→L | N→F |
|---|---|---|---|---|---|---|
| `EventConnection` | 30 767 | **47 538** | 7 294 | 5 618 | — | — |
| `LinkConnection` | 3 818 | **32 327** | 3 647 | 3 403 | 642 | 63 |
| `PropertyConnection` | 13 594 | 10 579 | 5 203 | 688 | — | — |

**`F→F` is the single largest bucket for events and links**: a bus routinely holds connections where
*neither* endpoint lives in its own partition — it is pure wiring between two foreign partitions.
`Levels/MP_001/MP_001.txt:12789-12796` wires two instances of `Levels/MP_001/StreetProps` to each
other from the level bus.

Two consequences, both correcting earlier reasoning in this document:

1. **A same-partition-scoped endpoint picker is not enough for connections.** The §6.2 tier-2 scan
   across loaded partitions is required, not optional, for connection authoring (Phase 4). It is
   still optional for `objects`/`components` appends.
2. **§7.2's dangling-endpoint detection cannot be local.** Deleting an instance in partition X can
   break connections held by an unrelated bus in partition Y, **with no back-reference in X to warn
   you**. The three-tier policy stands, but tier 2 ("warn, don't scan eagerly") is now the *common*
   case rather than the exotic one, and the tier-1 same-bus scan should be understood as catching a
   minority of real breakage. Budget for the full scan, and cache it per level load.

Also note the endpoint *field* accepts both renderings on the same slot — `Levels/MP_001/Conquest.txt:3117-3121`
has a bare-guid `Source` and a `<path>/<GUID>` `Target` in one struct. A picker must emit both forms.
...
**Verified from the Venice-EBX corpus (73 475 partition dumps):**

- **`objects` members never cross a partition boundary.** 0 of 195 440 members contain a `/`;
  18 738 of 18 738 resolved to a same-file instance on the `Objects/`+`Gameplay/`+`Levels/` subset.
- A raw entity (light, logic node) sits **directly** in `objects`; a `ReferenceObjectData` is only
  needed to instantiate another blueprint. Minimal light prefab = `PointLightEntityData` +
  `PrefabBlueprint` with one member
  (`Objects/CeilingLight_02/ceilinglight_02_yellow_dest_nongroupable_autogen.txt`).
  `PointLightEntityData` occurs 332× and `SpotLightEntityData` 147× as direct `objects` members.
- `ObjectBlueprint.object` (singular) is always a bare same-partition guid: **8 250 of 8 250, zero
  cross-partition, zero null.** (709 cross-partition `Object <path>/<GUID>` lines exist but belong to
  `VoiceOverGlobalNode.Object`, an unrelated type.)
- **`indexInBlueprint == array position` in `objects`: 2 861 of 2 861** over a 4 000-file sample —
  but see §7.4, this conflicts with the wiring doc's level-global claim.
- **`indexInBlueprint == 65535` (0xFFFF) is a "not in objects" sentinel**, present in 9 252 files.
- **`runtimeComponentCount != #components` in 366 cases** (18 163 equal). `Silo_M_01.txt:332-335`
  has 2 members / count 1; `GrowlerITV.txt:7078` has 19 members / count 99.
- **`PhysicsPartInfo.partComponentIndex` is a positional index into `components[]`**
  (`Objects/ShopShowcase_01.txt:524-549`), present in **6 577 files**.
- `PropertyTrackData.times` ↔ `values` are strict parallel arrays; no genuine length mismatch found.
- **The `InterfaceDescriptorData` is not in `objects`** — reachable only via
  `EntityBusData.descriptor`. It may be a subclass (`AssemblyDescriptorData`).
- `InterfaceDescriptorData` exposes six arrays of inline structs: `fields` (`DataField`, with
  `AccessType FieldAccessType_{Source,Target}` carrying direction), `inputEvents`/`outputEvents`
  (`DynamicEvent`, id only), `inputLinks`/`outputLinks` (`DynamicLink`, id only).
- **Connection endpoint locality**: `F→F` (both endpoints foreign to the bus) is the largest bucket
  for `EventConnection` (47 538) and `LinkConnection` (32 327). Full table in §7.5.
- **`LinkConnection.source` is the only nullable endpoint in vanilla data** — 705 occurrences,
  705/705 paired with `sourceFieldId 0`. `PropertyConnection`/`EventConnection` never null either
  endpoint; no connection ever nulls `target`.
- 0 exact-duplicate `PropertyConnection` tuples in 30 064 entries; connection arrays are not sorted
  (684 maximal same-source runs across 943 connections in a 500-file sample), i.e. order looks like
  authoring order.
- `objects` is not a unique field name: `HealthStateData.objects` (12 260 members) and
  `BreakablePartComponentData.objects` (3 958) are unrelated.
- No array of bare enums was found in 73 475 files. Every enum observed is a scalar field.
...
- That the engine tolerates a connection whose `source`/`target` is nil, or one pointing at a
  container no longer reachable from the bus (**G4**, still unobserved). Vanilla data attests
  `LinkConnection.source = nil` only; it says nothing about the other five slots or about dangles.
- That connection-array **order** is cosmetic. Inferred from absence of duplicates and absence of
  sorting; static dumps cannot observe runtime evaluation order. Treat order as
  preserve-on-round-trip and do not rely on reordering being safe.
- That `MeshVariationDatabaseEntry.materials[]` must match the mesh's material-slot count. The shape
  and the `(mesh, variationAssetNameHash)` keying are confirmed; the slot count lives in binary mesh
  assets outside the corpus.
- That the engine tolerates a stale `runtimeComponentCount` after an append/remove.
...
*(An earlier draft of this document assumed connection endpoints were predominantly same-partition,
and that `indexInBlueprint` should be treated as an identity rather than a position. **Both were
wrong** — see §7.5 and §7.4. They are recorded here so they are not re-assumed.)*

---

## 10. Open questions

**Answerable from the dumps alone — do these first, they are cheap and they change the design:**

| # | Question | Status |
|---|---|---|
| **Q6** | Are connection endpoints predominantly same-partition? | **ANSWERED: no.** `F→F` is the largest bucket for events and links (§7.5). The cross-partition scan is mandatory for connection authoring. |
| **Q10** | Is `indexInBlueprint` the array position, level-global, or position-plus-base? | **OPEN and blocking any reorder.** Two measurements disagree (§7.4). Re-run the position check restricted to `Levels/*/Buildings.txt`-style world parts. ~30 min of grepping. |
| **Q11** | Is the `PrefabBlueprint` guid == partition guid convention universal? | Observed once (§2.1b); useful for validation if it holds. Countable. |

**Only the running game can answer these.** Listed so another session can probe them with
`tools/e2e/mapeditor_e2e.py`. Q1–Q4 should be done **before** committing to the Phase-2 design.
...
| **Q2** | Does the array wrapper expose `remove` / `erase` / `insert` / `resize`? | On a runtime clone: inspect the array userdata's metatable, or call them in a `pcall` and log. |
| **Q3** | Is `clear()` + re-`add()` safe on a `MakeWritable`'d loaded array? | On a per-instance clone first, then on a shared blueprint. Watch for the changeset-18675 corruption class. |
...
| **Q5** | What does the engine do with a connection whose endpoint is nil / removed? (**G4**) | Null one endpoint of a capture-point connection; observe. Vanilla only ever nulls `LinkConnection.source`, so the other slots are genuinely untested territory. |
...
| **Q9** | Does the engine tolerate a stale `runtimeComponentCount` after an append? | Append a component without touching the count; observe. Vanilla ships 366 mismatches, so it plausibly does — but those are deliberate, not stale. |
...
### Phase 2 — real array mutation, per-instance only (≈8.5 d)
...
9. **`MutateArrayCommand`** (append/remove/move/duplicate) via the snapshot→clear→re-add primitive,
   ...
   **~2 d.**
9b. **Index-coupling fixups** (§2.1c): renumber `indexInBlueprint` (skipping the `65535` sentinel),
    fix up `physicsPartInfos[].partComponentIndex`, and refuse to mutate one half of a known
    parallel-array pair (`times`/`values`). **Gated on Q10.** **~1.5 d.** — *this is the item most
    likely to be skipped and most likely to cause silent corruption if it is.*
...
11. **Dangling-endpoint detection**: same-bus scan with the confirm dialog and the "remove
    connections too" path (§7.2 tier 1), plus an explicit on-demand cross-partition scan, since
    §7.5 shows most real breakage is cross-bus. **~1.5 d.**
...
13. **Construct-and-seed** a new container with explicit defaults (§5.4), plus **seed-from-donor**
    via `DeepClone` of a picked existing instance. Because `objects` is same-partition-only
    (§2.1b), the donor is *copied in*, never referenced. **~2 d.**
...
15. **Surface the `descriptor`** in the prefab's contents view — it is not in `objects` (§2.1b) and
    is invisible today. **~0.5 d.**
16. **Bake validation** — an e2e that appends a component, saves, bakes, and asserts the component
    is in the emitted partition (Q4). **~1 d.**

### Phase 4 — connection authoring (≈6 d) — this is G1/G5/G7

17. **Cross-partition endpoint scan** (§6.2 tier 2) — the new server RPC, paged over the chunked
    transport, cached per level load. Required by §7.5, not optional. **~1 d.**
18. **Connection editor**: endpoint picker + field picker from `getFields` on the
    concrete target + `hashQuick`, and a `targetType` dropdown for events. Must emit both the
    bare-guid and `<path>/<GUID>` endpoint forms. **~3 d.**
19. **`LevelInjector:CreateWorldParts` writes connections** onto the injected `WorldPartData`
    (G1). **~1 d.**
20. **Derive `isEvent/PropertyConnectionTarget`** from the connections that exist (G7) — last, as
    `entity-wiring-and-networking.md` §4 insists. **~1 d.**

### Phase 5 — blueprint-wide structural edits (≈4 d, gated)

21. **Ordered operation log** replacing the path-keyed override map, so Apply can replay structural
    ops in order (§5.1). **~2 d.**
22. **Structural Apply-to-Blueprint** — **gated on partition shadowing being proven**
    (`bake-pipeline.md` §5). Do not start before that. **~2 d.**

**Total ≈29 engineering days** for the full feature, plus ~1 day of in-game probing (§10 Q1–Q4) up
front. Of that, **Phases 0–1 (≈5.5 d)** deliver the reference picker and a safe "remove component"
with no format migration, no index renumbering, and no new failure mode — and Phase 0 alone
(**≈1.5 d**) closes G6 and makes every connection row readable.
